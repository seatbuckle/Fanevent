...
router.get("/", requireRole("admin"), async (req, res) => {
  try {
    const { status, reportType, reportCategory, page = 1, limit = 20 } = req.query;
...
router.patch("/:id/status", requireRole("admin"), async (req, res) => {
  try {
    ...
    const update = {
      status,
      reviewedBy: req.clerkUserId || null,
      reviewedAt: new Date(),
    };
    if (adminNotes) update.adminNotes = adminNotes;
    ...
    // Non-blocking notify to the reporter
    if (report.reporterClerkId) {
      notify({
        userId: report.reporterClerkId,
        ...
        link: "/my-dashboard", // tweak if you have a 'my reports' page
      });
    }
...
router.patch("/:id/notes", requireRole("admin"), async (req, res) => {
  try {
    ...
    if (!notes || typeof notes !== "string") {
      return res.status(400).json({ success: false, message: "Valid notes are required" });
    }

    const report = await Report.findByIdAndUpdate(
      req.params.id,
      { adminNotes: notes, reviewedBy: req.clerkUserId, reviewedAt: new Date() },
      { new: true }
    );
    ...
    res.json({ success: true, message: "Notes added successfully", report });
  } catch (error) {
...
router.get("/stats/overview", requireRole("admin"), async (req, res) => {
  try {
    const [
      ...
      Report.aggregate([{ $group: { _id: "$reportCategory", count: { $sum: 1 } } }]),
    ]);
